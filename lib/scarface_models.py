...
class Device(SNSCRUDMixin, models.Model):
    ...
    @PushLogger
    @DefaultConnection
    def send(self, push_message, extra_pload=[], connection=None):
        """
        :type connection: SNSConnection
        :param connection: the connection which should be used.
        if the argument isn't set there will be created a default connection
        ...
        if not self.is_registered:
            raise NotRegisteredException
        push_message = self.platform.format_payload(push_message, extra_pload)
        print ("push_message", push_message)
...
class Platform(SNSCRUDMixin, models.Model):
    ...
    def format_payload(self, data, extra_pload=[]):
        return self.strategy.format_payload(data, extra_pload)
