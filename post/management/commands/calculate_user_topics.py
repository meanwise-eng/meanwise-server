    ...
    def handle(self, *args, **options):
        ...
        for post in posts:
            ...
            for topic in post.topics.all():
                ...
                if user_topic:
                    posts = Post.objects.filter(topics__id=topic.id, poster=post.poster).order_by('-created_on')[:5]
                    serializer = PostSummarySerializer(posts, many=True)
                    user_topic.top_posts = serializer.data
                    user_topic.save()

                if user_topic_with_category:
                    posts_with_category = Post.objects.filter(topics__id=topic.id, poster=post.poster,
